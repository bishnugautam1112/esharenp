...
export function Lobby({ peerId, onInitialize, onJoin, error }: LobbyProps) {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col font-sans relative overflow-hidden transition-colors duration-300">
        ...
        <div className="max-w-md w-full space-y-8">
          <div className="text-center">
            <motion.div 
              ...
              className="w-16 h-16 bg-gradient-to-br from-primary to-purple-600 rounded-2xl mx-auto mb-6 flex items-center justify-center shadow-2xl shadow-primary/20"
            >
            ...
            <motion.h1 
              ...
              className="text-4xl font-bold tracking-tight mb-3 text-foreground"
            >
              KuraKani app
            </motion.h1>
            <motion.p 
          ...
          <motion.div 
            ...
            className="bg-card/80 backdrop-blur-xl border border-border rounded-3xl p-8 space-y-8 shadow-2xl"
          >
            {error && (
            ...
            {!peerId ? (
              ...
              >
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full bg-background border border-input pl-11 pr-4 py-3.5 rounded-xl text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring/50 transition-all"
                    />
                  </div>
                ...
                <button
                  ...
                  className="w-full bg-primary hover:bg-primary/90 text-primary-foreground py-3.5 rounded-xl font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
              ...
              >
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-2">
                    <code className="flex-1 bg-background border border-input px-4 py-3.5 rounded-xl text-sm font-mono text-primary overflow-x-auto">
                      {peerId}
                    </code>
                    <button
                      onClick={handleCopy}
                ...
                <div className="relative">
                  ...
                  <div className="relative flex justify-center text-xs uppercase">
                    <span className="bg-card px-4 text-muted-foreground font-medium tracking-wider">Connect</span>
                  </div>
                </div>

                {/* Join Section */}
                <div className="space-y-3">
                  <label className="text-sm font-medium text-foreground">Friend's Username</label>
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 bg-background border border-input px-4 py-3.5 rounded-xl text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring/50 transition-all"
                    />
                    <button
                      onClick={() => targetId && handleJoin(targetId)}
                      disabled={!targetId}
                      className="bg-primary text-primary-foreground hover:bg-primary/90 px-6 py-3.5 rounded-xl font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      Chat
                    </button>
                  </div>
                ...
                {recentConnections.length > 0 && (
                  <div className="space-y-3 pt-2">
                    <label className="text-sm font-medium text-foreground">Recent Friends</label>
                    <div className="flex flex-col gap-2">
                      {recentConnections.map(conn => (
                        <button 
                          key={conn}
                          onClick={() => handleJoin(conn)}
                          className="flex items-center justify-between text-sm text-muted-foreground hover:text-foreground transition-colors bg-background border border-input hover:border-border px-4 py-3 rounded-xl group"
                        >
      ...
      {/* Footer */}
      <footer className="w-full py-6 text-center z-10 border-t border-border bg-background/50 backdrop-blur-md">
